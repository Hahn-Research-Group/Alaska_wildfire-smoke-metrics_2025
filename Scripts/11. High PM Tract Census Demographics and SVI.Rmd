---
title: "WSVI of High Wildfire PM Exposure Tracts and Population-Weighted Mean PMs"
...
library(psych)        # For PCA, KMO, Bartlett’s test
library(GPArotation)  # For varimax rotation
library(factoextra)   # For scree plot & PCA visualization
library(nFactors)     # For parallel analysis
...
* Ranks and summary statistics for top 50% of Census Tracts in a Wildfire-specific Social Vulnerability Index (modeled after Jung et al., 2024) + top 50% of tracts of # of PM days above 9µg/m³
* WSVI demographic percentage distributions of high smoke (>=50% PM rank) and low smoke (>50% PM rank) Census tracts.
...
A composite rank was created from these two themes, resulting in a single Wildfire-Specific Social Vulnerability Index (WSVI).

Note: In this analysis, occupation data from the ACS (`C24010` codes) were classified into **Outdoor** and **Non-Outdoor** categories based on the nature of the work. This classification is used to assess potential exposure to environmental factors, including wildfire PM.
...
# WSVI and PM Tract Ranks

## Rank ID and Visualization

We next ranked Census tracts by the number of PM days over 9µg/m³, as well 
as ranking them by Wildfire Social Vulnerability Index scores in Themes 1 and 2. PM ranks were
...
and another in gradients of purple and orange.
...
pm_census_with_svi_rank <- census_svi_final %>%
  ...
  mutate(
    ...
    highlight = ifelse(pm_normalized_rank >= 0.5 & composite_svi_rank >= 0.5, "High Smoke Exposure + High Wildfire Social Vulnerability", "Other")
  )
...
highlighted <- pm_census_with_svi_rank[pm_census_with_svi_rank$highlight == "High Smoke Exposure + High Social Vulnerability", ]
...
High_PM_High_SVI_RedGray <- ggplot(pm_census_with_svi_rank, aes(x = pm_normalized_rank , y =composite_svi_rank, color = highlight)) +
  ...
  geom_text_repel(
    data = pm_census_with_svi_rank %>% filter(highlight == "High Smoke Exposure + High Social Vulnerability"),
    aes(label = NAME),
    size = 3 / aspect_ratio,
    color = "red", 
    max.overlaps = Inf, 
  ...
  scale_color_manual(values = c("High Smoke Exposure + High Wildfire Social Vulnerability" = "red", "Other" = "grey35")) +
  labs(
    x = "Wildfire Smoke Exposure Rank",
    y = "Wildfire Social Vulnerability Rank", 
    title = "High PM + High WSVI Communities in Alaska"
  ) +
...
## Demographic Summary of High SVI + High PM AK Census Tracts

The following summary provides insights into key population characteristics for Alaska's census tracts identified as having both high wildfire social vulnerability and high wildfire PM exposure:
...
The selected census tracts represent **`r round(percent_pop, 2)`%** of Alaska's total population. Within these tracts, the population size varies widely, ranging from **`r pop_range[1]`** to **`r pop_range[2]`** individuals per tract.
...
A bivariate map was then created of wildfire smoke exposure and WFSVI by Census tract. Due to the strong variability in Alaskan Census tract size, separate vectors were created of all Census tracts in the Anchorage Borough and for Census tracts in the Fairbanks North Star Borough. 
...
legend <- bi_legend(
  ...
  ylab = "Higher SVI Score",   # Y-axis label
