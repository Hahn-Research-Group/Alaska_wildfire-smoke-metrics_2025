---
title: "11. Population Person-Days"
...
This document details the methods used to calculated and visualize the person-days of exposure to PM2.5 from wildfire smoke (WFS) in each Alaskan census tract, by category.
...
* Medium: PM2.5 levels 9.1–35.4 µg/m³
* Heavy: PM2.5 levels >35.5 µg/m³
...
* Medium_PD: Person-days with Medium exposure.
* Heavy_PD: Person-days with Heavy exposure.
...
pm_census <- daily_pm_avg %>%
  ...
  mutate(
    ...
    Medium = ifelse(PM2.5 > 9 & PM2.5 < 35.4, 1, 0),
    Heavy = ifelse(PM2.5 >= 35.5, 1, 0),
...
pm_census <- pm_census %>%
  ...
  mutate(
    ...
    Medium_PD = Medium * TOTALPOP,
    Heavy_PD = Heavy * TOTALPOP,
...
pm_census_summary <- pm_census %>%
  pivot_longer(
    cols = c(No_Smoke_PD, Light_PD, Medium_PD, Heavy_PD, Any_Smoke_PD),
...
plot_person_days_vs_acres <- function(data, acres_burned, title, y_breaks = NULL) {
  ...
  ggplot() +
    ...
    scale_color_manual(
      values = c("Any Smoke" = "black", "Medium" = "darkorange", "Heavy" = "red"),
      breaks = c("Light", "Medium", "Heavy"),  # Order the legend manually
      name = "Smoke Density:"
    ) +
    scale_shape_manual(
      values = c("Any Smoke" = 16, "Medium" = 15, "Heavy" = 17),
      breaks = c("Light", "Medium", "Heavy"),  # Order the legend manually
      name = "Smoke Density:"
    ) +
    scale_linetype_manual(
      values = c("Any Smoke" = "solid", "Medium" = "dotted", "Heavy" = "dashed"),
      breaks = c("Light", "Medium", "Heavy"),  # Order the legend manually
      name = "Smoke Density:"
    ) +
...
med_heavy_title <- "Person-Days of Exposure to PM2.5 from Wildfire Smoke and Acres Burned in Alaska, 2003-2020"
...
### Medium and Heavy Smoke
<br>
```{r med heavy, echo = F, fig.width=8, fig.height=6, dpi=300}

# Plot Medium and Heavy Smoke + Acres Burned
pm_census_med_heav <- pm_census_summary %>%
  filter(Smoke_Category %in% c("Medium", "Heavy")) %>%
  rename(Person_Days = Total_Person_Days) 

plot_person_days_vs_acres(pm_census_med_heav, acres_burned, med_heavy_title)
# ggsave(filename="plot_med_hvy_final.png", dpi = 300, width = 10, height = 7)

```

### Any, Medium and Heavy Smoke
<br>
```{r all, echo = F, fig.width=8, fig.height=6, dpi=300}

# Plot Any Smoke, Medium, and Heavy with y-axis break
pm_census_any_med_heav <- pm_census_summary %>%
  filter(Smoke_Category %in% c("Any_Smoke", "Medium", "Heavy")) %>%
  rename(Person_Days = Total_Person_Days) %>%
  mutate(Smoke_Category = ifelse(Smoke_Category == "Any_Smoke", "Any Smoke", Smoke_Category))

plot_person_days_vs_acres(pm_census_any_med_heav, acres_burned, med_heavy_title)
...
pm_census_daily_long <- pm_census %>%
     select(Date, No_Smoke_PD, Light_PD, Medium_PD, Heavy_PD, Any_Smoke_PD) %>%  # Include Date
...
plot_weekly_graph <- function(data, year, title= "Alaska Wildfire Smoke Exposures by PM Density,", filter_categories = NULL) {
    ...
    data_filtered <- data %>%
        filter(year(Week) == year,  # Filter by year
               Smoke_Category %in% c("Light", "Medium", "Heavy"),
    ...
    ggplot(data_filtered, 
        ...
        scale_fill_manual(
            values = c("Light" = "yellow", "Medium" = "orange", "Heavy" = "red"),
            breaks = c("Light", "Medium", "Heavy"),  # Order the legend manually
...
plot_weekly_all_years <- function(data, title = "Alaska Wildfire Smoke Exposures by PM Density, 2003-2020", filter_categories = NULL) {
    ...
    data_filtered <- data %>%
        filter(
            Smoke_Category %in% c("Light", "Medium", "Heavy"),  # Focus on smoke categories
...
```{r weekly 2003 graph med heavy, echo = F, fig.width=8, fig.height=6, dpi=300}

plot_weekly_graph(pm_census_weekly_filtered, 2003, filter_categories = c("Medium", "Heavy"))

# For saving
# ggsave(filename="plot_weekly_2003_medheavy.png", dpi = 300, width = 10, height = 7)

```

### 2004 (Medium + Heavy)

```{r weekly 2004 graph med heavy, echo = F, fig.width=8, fig.height=6, dpi=300}

plot_weekly_graph(pm_census_weekly_filtered, 2004, filter_categories = c("Medium", "Heavy"))

# For saving
# ggsave(filename="plot_weekly_2004_medheavy.png", dpi = 300, width = 10, height = 7)

```

### 2005 (Medium + Heavy)

```{r weekly 2005 graph med heavy, echo = F, fig.width=8, fig.height=6, dpi=300}

plot_weekly_graph(pm_census_weekly_filtered, 2005, filter_categories = c("Medium", "Heavy"))

# For saving
# ggsave(filename="plot_weekly_2005_medheavy.png", dpi = 300, width = 10, height = 7)

```

### 2006 (Medium + Heavy)

```{r weekly 2006 graph med heavy, echo = F, fig.width=8, fig.height=6, dpi=300}

plot_weekly_graph(pm_census_weekly_filtered, 2006, filter_categories = c("Medium", "Heavy"))

# For saving
# ggsave(filename="plot_weekly_2006_medheavy.png", dpi = 300, width = 10, height = 7)

```

### 2007 (Medium + Heavy)

```{r weekly 2007 graph med heavy, echo = F, fig.width=8, fig.height=6, dpi=300}

plot_weekly_graph(pm_census_weekly_filtered, 2007, filter_categories = c("Medium", "Heavy"))

# For saving
# ggsave(filename="plot_weekly_2007_medheavy.png", dpi = 300, width = 10, height = 7)

```

### 2008 (Medium + Heavy)

```{r weekly 2008 graph med heavy, echo = F, fig.width=8, fig.height=6, dpi=300}

plot_weekly_graph(pm_census_weekly_filtered, 2008, filter_categories = c("Medium", "Heavy"))

# For saving
# ggsave(filename="plot_weekly_2008_medheavy.png", dpi = 300, width = 10, height = 7)

```

### 2009 (Medium + Heavy)

```{r weekly 2009 graph med heavy, echo = F, fig.width=8, fig.height=6, dpi=300}

plot_weekly_graph(pm_census_weekly_filtered, 2009, filter_categories = c("Medium", "Heavy"))

# For saving
# ggsave(filename="plot_weekly_2009_medheavy.png", dpi = 300, width = 10, height = 7)

```

### 2010 (Medium + Heavy)

```{r weekly 2010 graph med heavy, echo = F, fig.width=8, fig.height=6, dpi=300}

plot_weekly_graph(pm_census_weekly_filtered, 2010, filter_categories = c("Medium", "Heavy"))

# For saving
# ggsave(filename="plot_weekly_2010_medheavy.png", dpi = 300, width = 10, height = 7)

```

### 2011 (Medium + Heavy)

```{r weekly 2011 graph med heavy, echo = F, fig.width=8, fig.height=6, dpi=300}

plot_weekly_graph(pm_census_weekly_filtered, 2011, filter_categories = c("Medium", "Heavy"))
...
```{r weekly 2012 graph med heavy, echo = F, fig.width=8, fig.height=6, dpi=300}

plot_weekly_graph(pm_census_weekly_filtered, 2012, filter_categories = c("Medium", "Heavy"))

# For saving
# ggsave(filename="plot_weekly_2012_medheavy.png", dpi = 300, width = 10, height = 7)

```

### 2013 (Medium + Heavy)

```{r weekly 2013 graph med heavy, echo = F, fig.width=8, fig.height=6, dpi=300}

plot_weekly_graph(pm_census_weekly_filtered, 2013, filter_categories = c("Medium", "Heavy"))

# For saving
# ggsave(filename="plot_weekly_2013_medheavy.png", dpi = 300, width = 10, height = 7)

```

### 2014 (Medium + Heavy)

```{r weekly 2014 graph med heavy, echo = F, fig.width=8, fig.height=6, dpi=300}
...
```{r weekly 2015 graph med heavy, echo = F, fig.width=8, fig.height=6, dpi=300}

plot_weekly_graph(pm_census_weekly_filtered, 2015, filter_categories = c("Medium", "Heavy"))

# For saving
# ggsave(filename="plot_weekly_2015_medheavy.png", dpi = 300, width = 10, height = 7)

```

### 2016 (Medium + Heavy)

```{r weekly 2016 graph med heavy, echo = F, fig.width=8, fig.height=6, dpi=300}

plot_weekly_graph(pm_census_weekly_filtered, 2016, filter_categories = c("Medium", "Heavy"))

# For saving
# ggsave(filename="plot_weekly_2016_medheavy.png", dpi = 300, width = 10, height = 7)

```

### 2017 (Medium + Heavy)

```{r weekly 2017 graph med heavy, echo = F, fig.width=8, fig.height=6, dpi=300}

plot_weekly_graph(pm_census_weekly_filtered, 2017, filter_categories = c("Medium", "Heavy"))

# For saving
# ggsave(filename="plot_weekly_2017_medheavy.png", dpi = 300, width = 10, height = 7)

```

### 2018 (Medium + Heavy)

```{r weekly 2018 graph med heavy, echo = F, fig.width=8, fig.height=6, dpi=300}

plot_weekly_graph(pm_census_weekly_filtered, 2018, filter_categories = c("Medium", "Heavy"))

# For saving
# ggsave(filename="plot_weekly_2018_medheavy.png", dpi = 300, width = 10, height = 7)

```

### 2019 (Medium + Heavy)

```{r weekly 2019 med heavy, echo = F, fig.width=8, fig.height=6, dpi=300}

plot_weekly_graph(pm_census_weekly_filtered, 2019, filter_categories = c("Medium", "Heavy"))

# For saving
# ggsave(filename="plot_weekly_2019_medheavy.png", dpi = 300, width = 10, height = 7)
...
```{r weekly 2020 graph med heavy, echo = F, fig.width=8, fig.height=6, dpi=300}

plot_weekly_graph(pm_census_weekly_filtered, 2020, filter_categories = c("Medium", "Heavy"))

# For saving
# ggsave(filename="plot_weekly_2020_medheavy.png", dpi = 300, width = 10, height = 7)

```

### 2003-2020 (Medium + Heavy)

```{r weekly 2003 2020 graph med heavy, echo = F, fig.width=8, fig.height=6, dpi=300}

plot_weekly_all_years(pm_census_weekly_filtered, filter_categories = c("Medium", "Heavy"))

# For saving
# ggsave(filename="plot_weekly_all_medheavy.png", dpi = 300, width = 10, height = 7)

```

### 2003-2020 Grid Plot (Medium and Heavy)
...
# Clean data to avoid missing/non-finite values
  ...
  ggplot(data_filtered, 
    ...
    geom_area(position = "stack", alpha = 0.6, color = "white") +
    scale_fill_manual(
      values = c("Medium" = "orange", "Heavy" = "red"),
      breaks = c("Medium", "Heavy"),
      name = "Smoke Density:"
    ) +
...
### High WF Year Grid Plot (Medium and Heavy)
...
# Save the final plot
# ggsave("Grid_PM25_High_WF.png", plot = facet_highwf, dpi = 300, width = 16, height = 10)
...
We performed t-tests comparing statewide PM2.5 averages, as well as Light, Medium, 
and Heavy person-days of wildfire smoke exposure for 2003–2011 and 2012–2020. 
...
There was not a statistically significant difference in medium and heavy person-days of wildfire smoke exposure from 2003-2011 to 2012-2020. 
...
exposure_levels <- pm_census_summary %>%
  filter(Smoke_Category %in% c("Light", "Medium", "Heavy")) %>%
...
kable(
  t_test_summary_pd,
  caption = "T-test Results for Light, Medium, and Heavy Person-Days of Smoke Exposure (First 9 Years vs Last 9 Years)"
)
...
Below is a table summarizing the total, medium, and heavy person-days of wildfire smoke exposure by census tract. Following this, a second table summarizes the number of census tracts that experienced increases or decreases in total person-days of exposure, as well as exposure to medium and high smoke densities.
...
census_change <- pm_census %>%
  ...
  summarise(
    ...
    First_9_Years_Medium = sum(Medium_PD[Year %in% 2003:2011], na.rm = TRUE),
    Last_9_Years_Medium = sum(Medium_PD[Year %in% 2012:2020], na.rm = TRUE),
    Medium_Change = Last_9_Years_Medium - First_9_Years_Medium,
    First_9_Years_Heavy = sum(Heavy_PD[Year %in% 2003:2011], na.rm = TRUE),
    Last_9_Years_Heavy = sum(Heavy_PD[Year %in% 2012:2020], na.rm = TRUE),
    Heavy_Change = Last_9_Years_Heavy - First_9_Years_Heavy,
...
# Summary table broken down by Total, Medium, and Heavy changes
summary_change <- census_change %>%
  summarise(
    ...
    Medium_Days_Increased = sum(ifelse(Medium_Change > 0, 1, 0)),
    Medium_Days_Decreased = sum(ifelse(Medium_Change <= 0, 1, 0)),
    Heavy_Days_Increased = sum(ifelse(Heavy_Change > 0, 1, 0)),
    Heavy_Days_Decreased = sum(ifelse(Heavy_Change <= 0, 1, 0))
  ) %>%
  pivot_longer(
    cols = everything(),
    names_to = "Category",
...
summary_table <- datatable(
  ...
  caption = "Summary of Census Tracts by Status of Change (Total, Medium, Heavy)"
)
...
# # Group by NAME and calculate total person-days for each period
# pd_first_last <- pm_census %>%
#   filter(Year %in% c(first_9_years, last_9_years)) %>%
#   group_by(NAME, Period = ifelse(Year %in% first_9_years, "First 9 Years", "Last 9 Years")) %>%
#   summarise(Total_PD = sum(Any_Smoke_PD, na.rm = TRUE)) %>%
#   pivot_wider(names_from = Period, values_from = Total_PD, values_fill = 0) %>%
#   mutate(
#     Change = `Last 9 Years` - `First 9 Years`,
#     Status = ifelse(Change > 0, "Increased", "Decreased")
#   )
# 
# # Summary of the number of census tracts with increased or decreased person-days
# summary_table <- pd_first_last %>%
#   group_by(Status) %>%
#   summarise(Count = n())
# 
# # Display the detailed table with DT
# datatable(
#   pd_first_last,
#   caption = "Detailed Census Tracts with Person-Days Change Over Periods",
#   options = list(
...
# datatable(
#   summary_table,
#   caption = "Summary of Census Tracts by Status of Change",
#   options = list(
#     pageLength = 5,
#     autoWidth = TRUE,
#     dom = 't' # Only display the table (no search, pagination, or other controls)
#   )
# )
