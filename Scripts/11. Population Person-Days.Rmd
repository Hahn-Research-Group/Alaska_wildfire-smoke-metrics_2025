...
# Step 3: Summarize person-days for Medium and Heavy exposure levels
exposure_summary <- pm_census_filtered %>%
  select(Year, Month, Week, Medium, Heavy, Any_Smoke) %>%
  group_by(Year, Month, Week) %>%
  summarize(
    Medium_Days = sum(Medium, na.rm = TRUE),
    Heavy_Days = sum(Heavy, na.rm = TRUE),
    Any_Days = sum(Any_Smoke, na.rm = TRUE),
    .groups = "drop"
  )

start_end_summary <- exposure_summary %>%
  pivot_longer(cols = c(Any_Days, Medium_Days, Heavy_Days), names_to = "Exposure_Level", values_to = "Days") %>%
  filter(Days > 0) %>%
